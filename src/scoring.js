...
export function computeBaseline(media, method = 'median') {
    const views = media
        .map((item) => item.views)
...
export function scoreMedia({ media, creator = {}, baselineViews = null, now = new Date() }) {
    const followers = toNumber(creator.followers ?? media.followers);
    const views = toNumber(media.views);
    const likes = toNumber(media.likes);
    const comments = toNumber(media.comments);
    const shares = toNumber(media.shares);
    const saved = toNumber(media.saved);
    const days = media.postedAt ? ageDays(media.postedAt, now) : null;
    ...
    const engagementProxy = views > 0 ? (likes + comments * 5 + shares * 8 + saved * 3) / views : null;
